...
assert.match(css, /\.reader-shell\.is-searching \.search-results\s*\{[\s\S]*border:\s*1px solid var\(--reader-glass-edge\)[\s\S]*box-shadow:\s*0 8px 24px/, "search results should be one unified low-lift glass list");
...
assert.match(css, /\.paper-nav-item\[aria-current="true"\] \.paper-nav-marker\s*\{[\s\S]*background:\s*var\(--reader-red\)/, "active paper should use a dark red state marker without a side stripe");
...
assert.match(css, /\.section-line:hover::before/, "collapsed section index should support hover line focus");
...
const sectionLineVisualRule = css.match(/\.section-line::before\s*\{(?<body>[\s\S]*?)\n\}/)?.groups?.body ?? "";
const sectionLineHoverRule = css.match(/\.section-line:hover::before,\s*\n\.section-line:focus-visible::before\s*\{(?<body>[\s\S]*?)\n\}/)?.groups?.body ?? "";
const sectionActiveRule = css.match(/\.section-line\.is-active::before\s*\{(?<body>[\s\S]*?)\n\}/)?.groups?.body ?? "";
...
assert.match(sectionLineRule, /width:\s*44px/, "section rail controls should have a usable hit target");
assert.match(sectionLineRule, /height:\s*32px/, "section rail controls should have a usable hit target height");
assert.match(sectionLineVisualRule, /width:\s*18px/, "section line visual should keep a quiet base length");
assert.match(sectionLineVisualRule, /background:\s*var\(--reader-section-line\)/, "section line visuals should use theme-aware contrast tokens");
assert.match(sectionLineVisualRule, /box-shadow:\s*0 0 0 1px var\(--reader-section-line-glow\)/, "section lines should use a subtle dark-mode visibility edge");
...
assert.match(sectionActiveRule, /width:\s*32px/, "active section should remain distinguishable without color alone");
...
assert.match(css, /\.paper-header\s*\{[\s\S]*max-width:\s*min\(940px,\s*calc\(100vw - 40px\)\)/, "paper header should leave room for a source-backed visual abstract");
assert.match(css, /\.chunk-list\s*\{[\s\S]*max-width:\s*min\(940px,\s*calc\(100vw - 40px\)\)/, "chunk list should leave room for editorial research plates");
assert.match(css, /\.paper-copy,[\s\S]*max-width:\s*75ch/, "paper prose should retain a readable line measure inside the wider editorial canvas");
assert.match(css, /\.source-paragraph,[\s\S]*max-width:\s*75ch/, "source prose should retain a readable line measure inside the wider editorial canvas");
