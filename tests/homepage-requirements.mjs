...
assert.match(body, /MeshoptDecoder[\s\S]*setMeshoptDecoder\(MeshoptDecoder\)/, "homepage should configure the decoder required by the optimized brain model");
...
assert.ok(modelStats.size > 2_000_000, "local brain model should remain a substantial binary asset, not a tiny placeholder");
assert.ok(modelStats.size < 4_000_000, "local brain model should retain the optimized payload after the visual rollback");
