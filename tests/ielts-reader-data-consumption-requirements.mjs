...
assert.match(renderers, /function renderSprintPriorities/);
assert.match(renderers, /滚动优先级/);
assert.match(renderers, /prioritySystem/);
...
assert.equal(getSprintDay(data.sprintPlan, "2026-08-15").template, "rollingStandard");
...
assert.match(sprintHtml, /滚动优先级/);
assert.match(sprintHtml, /P0 · 口语就绪/);
assert.match(sprintHtml, /优先顺序 1/);
assert.match(sprintHtml, /依据：/);
assert.match(sprintHtml, /8月15日 起/);
assert.match(sprintHtml, /4\.5 小时/);
assert.match(sprintHtml, /8月10日–8月11日/);
assert.match(sprintHtml, /跨日结算/);
assert.match(sprintHtml, /完整复盘留到 8 月 13 日/);
