...
assert.match(html, /--brush-font:[\s\S]*?"HanziPen SC"[\s\S]*?"Kaiti SC"[\s\S]*?"Songti SC"[\s\S]*?serif;/, "papers homepage should define a local brush-style Chinese font stack");
...
assert.match(html, /h1\s*\{[\s\S]*?font-family:\s*var\(--brush-font\);[\s\S]*?writing-mode:\s*vertical-rl;[\s\S]*?text-orientation:\s*upright;[\s\S]*?white-space:\s*nowrap/, "desktop 文献阁 title should use the brush font stack and render as a vertical right-side axis with upright Chinese text");
...
assert.match(html, /\.project-card::after\s*\{[\s\S]*?radial-gradient\(circle at 50% 24px,[\s\S]*?linear-gradient\(90deg,\s*transparent calc\(50% - 0\.65px\),\s*rgba\(112,\s*38,\s*34,\s*0\.4\)/, "bookmark cards should draw a top hole and a short thread cue with CSS");
assert.match(html, /\.project-card::after\s*\{[\s\S]*?linear-gradient\(90deg,\s*transparent 0 48%,\s*rgba\(24,\s*60,\s*73,\s*0\.09\) 49% 51%,\s*transparent 52% 100%\)/, "bookmark cards should include a subtle vertical center crease");
assert.match(html, /\.project-card h2\s*\{[\s\S]*?color:\s*var\(--dai-blue\);[\s\S]*?font-family:\s*var\(--brush-font\);[\s\S]*?writing-mode:\s*vertical-rl;[\s\S]*?text-orientation:\s*upright;[\s\S]*?white-space:\s*nowrap/, "project card titles should use the brush font stack with dai-blue upright Chinese vertical text");
