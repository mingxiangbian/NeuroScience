---
id: terms-further-reading
title: 术语速查与延伸
status: not-started
learning_progress: 0
last_updated: 2026-07-13
priority: high
decision_role: 作为跨模块术语和资料索引，支持即时查阅与继续学习。
graph_role: support
knowledge_relations: []
---

## 目标

把常见术语、公式和后续资料放在一个可全文检索的模块中，支持学习过程中的即时查阅。

## 核心知识

# 22. 术语速查表

| 术语 | 含义 |
|---|---|
| Asset | 资产 |
| Liability | 负债 |
| Equity | 股东权益 |
| Revenue | 收入 |
| Gross Profit | 毛利润 |
| Operating Income | 营业利润 |
| Net Income | 净利润 |
| EPS | 每股收益 |
| CFO | 经营活动现金流 |
| CapEx | 资本支出 |
| FCF | 自由现金流 |
| Market Cap | 总市值 |
| Enterprise Value | 企业价值 |
| P/E | 市盈率 |
| P/B | 市净率 |
| PEG | 市盈率相对盈利增长比率 |
| Earnings Yield | 盈利收益率 |
| FCF Yield | 自由现金流收益率 |
| ROE | 净资产收益率 |
| ROIC | 投入资本回报率 |
| Dividend | 分红 |
| Buyback | 股票回购 |
| Dilution | 股权稀释 |
| ETF | 交易所交易基金 |
| Index Fund | 指数基金 |
| Expense Ratio | 基金费用率 |
| Bid | 买方最高报价 |
| Ask | 卖方最低报价 |
| Spread | 买卖价差 |
| Volatility | 波动率 |
| Drawdown | 回撤 |
| Diversification | 分散投资 |
| Asset Allocation | 资产配置 |
| Rebalancing | 再平衡 |
| DCF | 折现现金流 |
| WACC | 加权平均资本成本 |
| Terminal Value | 终值 |
| Margin | 保证金或利润率，需结合语境 |
| Leverage | 杠杆 |
| Liquidity | 流动性 |
| Dollar-Cost Averaging | 定期定额投入 |

---

# 23. 延伸学习资料

以下资料主要用于基础教育和概念核对。不同国家和地区的法律、账户、税务与产品规则可能不同。

1. **Investor.gov — Investing Basics**
   https://www.investor.gov/introduction-investing/investing-basics

2. **Investor.gov — Compound Interest Calculator**
   https://www.investor.gov/financial-tools-calculators/calculators/compound-interest-calculator

3. **Investor.gov — Index Funds**
   https://www.investor.gov/introduction-investing/investing-basics/investment-products/mutual-funds-and-exchange-traded-4

4. **Investor.gov — Dollar-Cost Averaging**
   https://www.investor.gov/introduction-investing/investing-basics/glossary/dollar-cost-averaging

5. **Investor.gov — P/E Ratio**
   https://www.investor.gov/introduction-investing/investing-basics/glossary/price-earnings-pe-ratio

6. **Investor.gov — Investment Fraud Red Flags**
   https://www.investor.gov/protect-your-investments/fraud/how-avoid-fraud/red-flags-investment-fraud-checklist

7. **FINRA — Investing Basics**
   https://www.finra.org/investors/investing/investing-basics

8. **FINRA — Asset Allocation and Diversification**
   https://www.finra.org/investors/investing/investing-basics/asset-allocation-diversification

9. **FINRA — Buying and Selling Investments**
   https://www.finra.org/investors/investing/investing-basics/buying-and-selling

10. **CFA Institute — Free Cash Flow Valuation**
    https://www.cfainstitute.org/insights/professional-learning/refresher-readings/2026/free-cash-flow-valuation

---

# 最后的核心原则

把整篇教程压缩成十句话：

1. 先建立财务安全垫，再承担投资风险。
2. 不使用近期必需资金投资高波动资产。
3. 不理解的东西不买。
4. 收益必须与风险、费用、税务和通胀一起看。
5. 分散不是买很多代码，而是分散真正的风险来源。
6. 好公司不等于任何价格都值得买。
7. 估值指标用于比较和提问，不是自动交易信号。
8. DCF 的价值在于暴露假设，而不是制造精确幻觉。
9. 长期纪律、持续投入和费用控制通常比频繁预测更可控。
10. 一笔投资失败不可怕，没有流程地重复犯错才可怕。

## 学习记录

- 搜索一个陌生术语，先写定义，再回到它所属的概念模块。
- 给延伸阅读标记“已读、待读或仅作索引”，避免把链接数量当作理解。

## 知识笔记

### 术语速查要回到语境

这是一张可在阅读器中高亮和写本地批注的概念卡；它用于复述和检索，不替代原文中的完整说明。

#### 核心定义

术语表提供简短定义和检索入口，延伸资料提供继续阅读的方向。

#### 核心机制

同一个指标在不同资产、行业和会计口径下可能含义不同，因此术语需要回到原始问题和数据语境。

#### 逐步示例

搜索 P/E 后，回到估值模块检查它的分子分母、假设和不适用情形。

#### 边界与常见错误

只背定义、不检查使用条件，或把外部资料视为自动可靠，会造成表面熟悉。

#### 一句话总结

术语表用于定位，完整理解来自回到相关概念和证据。
