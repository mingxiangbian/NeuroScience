"""Synthetic EXP-083 input, serial execution, adapter and fail-closed checks."""
import json
...
import run_preload_diagnostic as driver
import preload_diagnostic_child as child


def test_namespace_adapter_only_changes_journal_path_without_importing_model():
    before = child.OBSERVER.read_bytes()
    observer = child.load_observer()
    assert observer.STAGE_PATH == child.STAGE_PATH
    assert observer.STAGE_PATH != observer.ROOT / "private/validation/exp-082/attempt-1/stages.jsonl"
    assert observer.ROOT == child.ROOT
    assert child.OBSERVER.read_bytes() == before
    assert not any(name.split(".")[0] in {"mlx", "torch", "transformers", "mlx_lm"} for name in sys.modules)


def test_adapter_rejects_drift_before_import(monkeypatch, tmp_path):
    source = tmp_path / "wrong.py"
    source.write_text("raise AssertionError('must not execute')\n")
    monkeypatch.setattr(child, "OBSERVER", source)
    with pytest.raises(RuntimeError, match="inherited_observer_identity"):
        child.load_observer()
...
def result(mode, ordinal):
    ...
    return {"prediction": [0] * 6, "route_requested": routed, "hypothetical_route": ordinal == 6,
            "fallback_reason": None, "used_path": "m3" if routed else "m1",
            "resources": {"peak_rss_bytes": 1024, "mlx_peak_bytes": 0},
            "counters": {"m1_attempts": 1, "m1_cache_hit": 0, "m3_attempts": int(routed),
...
def execute_fixture(tmp_path, fixture_value, **kwargs):
    ...
    with (tmp_path / "results.jsonl").open("x") as stream:
        outcome = driver.execute(monitor, records, source_rows, factory, driver.time.monotonic() + 150,
                                 stream, lambda _: None, **kwargs)
    return outcome, [json.loads(line) for line in (tmp_path / "results.jsonl").read_text().splitlines()]
...
def test_failed_research_constructor_does_not_borrow_m1_exit(monkeypatch, tmp_path):
    ...
    assert driver.process_events(monitor.events_path)[0]["job_id"] == "exp083-m1-prelude"
