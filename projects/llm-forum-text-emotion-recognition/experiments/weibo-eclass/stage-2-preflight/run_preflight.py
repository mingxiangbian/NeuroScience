#!/usr/bin/env python3
"""Run the train-only EXP-041 Stage 2 model-stack preflight."""
...
def load_config() -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("EXP-041 must remain a registered Minor experiment")
    if config["data"]["validation_access"] or config["data"]["test_access"]:
        raise ValueError("EXP-041 must remain train-only")
...
def load_run(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise FileNotFoundError("Run static before later EXP-041 stages")
...
def run_qwen(config: dict[str, Any]) -> None:
    ...
    with temporary.open("w", encoding="utf-8") as target:
        ...
        for index, (row, view, thinking, condition) in enumerate(cases):
            ...
            seed_namespace = inference["generation_seed_namespace"]
            seed = int(sha256_text(f"{seed_namespace}:{condition}:{row['sample_id']}")[:8], 16)
            mx.random.seed(seed)
    ...
    report = {
        ...
        "sampler": {
            "generation_seed_namespace": inference["generation_seed_namespace"],
