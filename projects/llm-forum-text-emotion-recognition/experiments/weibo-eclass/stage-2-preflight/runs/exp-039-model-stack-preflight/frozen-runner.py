#!/usr/bin/env python3
"""Run the train-only EXP-039 Stage 2 model-stack preflight."""
...
def load_config() -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("EXP-039 must remain a registered Minor experiment")
    if config["data"]["validation_access"] or config["data"]["test_access"]:
        raise ValueError("EXP-039 must remain train-only")
...
def load_run(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise FileNotFoundError("Run static before later EXP-039 stages")
    ...
    *,
    ...
    if include_target:
        messages.append(
            {
                "role": "assistant",
                "content": prompt["assistant_target_template"].format(label=row["label"]),
            }
        )
    return messages
...
def run_static(config: dict[str, Any]) -> None:
    ...
    for row in selected:
        ...
        target_json = prompt["assistant_target_template"].format(label=row["label"])
