#!/usr/bin/env python3
"""Independently verify the train-only EXP-039 Stage 2 preflight."""
...
def load_and_verify_config() -> dict[str, Any]:
    ...
    if config.get("experiment_id") != EXPERIMENT_ID or config.get("tier") != "Minor":
        raise ValueError("Unexpected EXP-039 identity")
...
def verify_privacy(
    ...
    if (private.stat().st_mode & 0o777) != 0o700:
        raise PermissionError("Private EXP-039 root is not mode 0700")
