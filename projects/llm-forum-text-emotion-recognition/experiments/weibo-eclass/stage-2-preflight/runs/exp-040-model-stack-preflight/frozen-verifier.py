#!/usr/bin/env python3
"""Independently verify the train-only EXP-040 Stage 2 preflight."""
...
def load_and_verify_config() -> dict[str, Any]:
    ...
    if config.get("experiment_id") != EXPERIMENT_ID or config.get("tier") != "Minor":
        raise ValueError("Unexpected EXP-040 identity")
    ...
    if "from run_preflight import" in verifier_source or "from label_parser import" in verifier_source:
...
def verify_privacy(
    ...
    if (private.stat().st_mode & 0o777) != 0o700:
        raise PermissionError("Private EXP-040 root is not mode 0700")
