#!/usr/bin/env python3
"""Independently verify EXP-046 without importing its runner."""
...
def expected_selection(
    ...
    for row in rows:
        ...
        if not prompt_ids or not all(
            isinstance(token, int) and not isinstance(token, bool) for token in prompt_ids
        ):
            raise TypeError("Independent tokenizer contract did not return integer token IDs")
        measured.append(
            {
                "label": row["label"],
                "messages": messages,
...
def main() -> None:
    ...
    if output.exists():
        raise FileExistsError("EXP-046 verification is append-only")
