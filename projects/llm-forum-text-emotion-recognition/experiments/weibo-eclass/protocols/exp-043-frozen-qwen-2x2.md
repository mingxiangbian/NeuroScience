...
EXP-043 completed all 5,088 frozen validation generations and passed independent
verification with 10 checks, zero mismatches and no test access.

| Condition | View | Reasoning | Macro-F1 | Accuracy | Weighted-F1 | Parser valid |
| --- | --- | --- | ---: | ---: | ---: | ---: |
| A | target only | off | 0.308684 | 0.230346 | 0.239951 | 0.999214 |
| B | previous context | off | 0.281480 | 0.188679 | 0.176090 | 0.999214 |
| C | target only | on | 0.333818 | 0.222484 | 0.202040 | 0.918239 |
| D | previous context | on | 0.317997 | 0.219340 | 0.194241 | 0.908019 |

The frozen primary-metric rule selected condition C. This is not a general claim
that C is the best deployable condition: compared with A, it has lower Accuracy
and Weighted-F1, lower parser validity and much higher generation cost.

- Average context effect: `-0.021512`, 95% paired-bootstrap CI
  `[-0.037515, -0.006905]`.
- Average reasoning effect: `+0.030825`, 95% CI `[+0.007225, +0.057146]`.
- Interaction: `+0.011383`, 95% CI `[-0.015119, +0.036247]`.
- Selected C minus EXP-042 M2 target-only Macro-F1: `-0.261107`, descriptive
  validation comparison only.
- Reasoning-off conditions generated 7,962/7,852 tokens in 15.39/15.99 minutes;
  reasoning-on conditions generated 664,710/686,390 tokens in 6.54/6.25 hours.
  Peak memory was 10.02 GB and API cost was USD 0.
- On 332 `first_clause` controls the paired prompt hashes matched exactly. The
  reasoning-on raw outputs were not exactly repeatable across the two batched
  condition runs, so this control is recorded as a runtime reproducibility caveat,
  not as evidence of a context effect.

The generated report was written before independent verification and therefore
retains its pre-verification status line. The definitive status is recorded in
`verification.json` and `run.json` under the immutable run directory.
