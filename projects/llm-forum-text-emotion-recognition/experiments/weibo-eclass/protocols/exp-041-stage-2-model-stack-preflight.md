# EXP-041: Weibo EClass Stage 2 Model-Stack Preflight
...
parent_experiment: EXP-040
---

## Observed Outcome

All seven train-only smoke stages passed. Static selection reconstructed 56
rows across all 14 label-by-context strata; both tokenization paths preserved
every target. M1 produced finite seven-class decision scores for both views,
and M2 completed two finite optimizer steps with updated classifier parameters.

Qwen strict parseability was 37/42 (88.1%), above the frozen 80% gate. All
21 non-thinking generations were valid. Sixteen of 21 thinking generations
were valid; the other five reached the 1024-token cap before `</think>`. Every
condition retained at least four valid outputs. This is a format/resource smoke
result, not emotion-classification accuracy.

The exact LoRA plan inserted 112 modules across blocks 20-35. The saved adapter
contained 224 tensors and 7,340,032 trainable parameters; all 112 LoRA B tensors
were nonzero after two optimizer steps. Reloaded logits were finite and local
peak memory was 8.65 GB. No validation or test row was accessed, and no task
performance metric was computed. Independent amendment `AUDIT-EXP-041-V1`
passed 16 checks with zero mismatch.

## Correction From EXP-040

`EXP-040` completed all 42 Qwen generations but failed the 80% strict-parser
gate at 28/42. All 21 non-thinking generations parsed successfully. All 14
parser failures were thinking generations that reached the common 384-token
cap before emitting `</think>`; the seven thinking generations that stopped
normally parsed successfully.

This rerun changes only the common generation cap from 384 to 1024 tokens. The
same train-only rows and the exact `EXP-040` per-row generation seeds are
retained so the first 384 sampled tokens remain controlled. Data, models,
prompt, parser, sampler distribution, resource budget and pass criteria remain
unchanged. `EXP-040` and its frozen implementation remain preserved as failed
evidence.
...
min-p 0 and 1024 maximum new tokens. The thinking condition may emit reasoning
