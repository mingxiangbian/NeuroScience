# EXP-040: Weibo EClass Stage 2 Model-Stack Preflight
...
parent_experiment: EXP-039
---

## Observed Outcome

`EXP-040` passed the static, model, tokenization, classical and encoder smoke
stages, then failed the frozen Qwen parseability gate. The non-thinking path
was strictly valid for all 21/21 generations. In the thinking path, 14/21
generations reached the common 384-token cap; every one of those 14 outputs
ended before the required `</think>` boundary and therefore failed parsing.
The remaining 7/21 thinking generations parsed successfully. Overall strict
validity was 28/42 (66.7%), below the registered 80% gate.

No validation or test row was accessed, no task-performance metric was
computed, and the LoRA stage was not started. The failure identifies an
insufficient generation budget for the thinking smoke, not a label-schema or
non-thinking parser failure. A corrected run requires a new experiment ID.

## Correction From EXP-039

`EXP-039` failed its first static check because the assistant JSON target was
rendered with `str.format`, which interpreted the outer JSON braces as a field.
This rerun changes only target rendering to a checked literal `{label}`
replacement and adds a regression test that constructs an assistant training
message. Data, models, prompt text, parser, resource budget and all pass criteria
remain unchanged. The original selection salt is retained intentionally so the
corrected run uses the same deterministic train-only smoke rows.
