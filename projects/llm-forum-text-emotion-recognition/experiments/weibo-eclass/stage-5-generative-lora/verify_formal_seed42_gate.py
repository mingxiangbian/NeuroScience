#!/usr/bin/env python3
"""Independently verify the EXP-047 seed-42 formal execution gate."""
...
VERIFICATION_ID = "EXP-047-FORMAL-SEED-42-GATE-VERIFY-V1"
...
def recompute(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    dry_verifier = resolve_project_path(contract["frozen_artifacts"]["dry_run_verifier"]["path"])
    test_path = resolve_project_path(contract["implementation"]["test"]["path"])
    checks = {
        "dry_run_verifier": run_checked([str(python), str(dry_verifier), "--check"]),
...
def build_verification(contract: dict[str, Any], completed_at_utc: str) -> dict[str, Any]:
    return {
        ...
        "next_required_step": "Bind a separate seed-42 train-only authorization and execute once.",
