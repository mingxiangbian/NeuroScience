#!/usr/bin/env python3
"""Independently verify the corrected EXP-047 seed-43 attempt-2 run."""
...
VERIFICATION_ID = "EXP-047-FORMAL-SEED-43-ATTEMPT-2-TRAIN-VERIFY-V1"
...
def verify_authorization(contract: dict[str, Any], report: dict[str, Any]) -> dict[str, Any]:
    ...
    expected = {
        ...
        "gate_verification_id": "EXP-047-FORMAL-SEED-43-ATTEMPT-2-GATE-VERIFY-V1",
    ...
    require_equal(
        ...
        {"seeds": [43], "test_split": False, "train": True, "validation_split": False},
        "Authorization scope drift",
    )
    gate = load_json(gate_path)
    if (
        gate.get("verification_id") != "EXP-047-FORMAL-SEED-43-ATTEMPT-2-GATE-VERIFY-V1"
...
def verify_adapter(
    ...
    expected_config = {**runtime, "config": str(resolve_project_path(contract["frozen_artifacts"]["runtime_seed_43"]["path"]))}
...
def build_verification(contract: dict[str, Any], completed_at_utc: str) -> dict[str, Any]:
    ...
    return {
        ...
        "next_required_step": "Run the frozen two-pass singleton replay for seed 43; seed 44 and validation/test remain closed.",
        "recalculated": {
            "adapter": adapter,
