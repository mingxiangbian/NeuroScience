#!/usr/bin/env python3
"""Independently verify the corrected EXP-047 seed-43 attempt-2 gate."""
...
CONTRACT_ID = "EXP-047-FORMAL-SEED-43-ATTEMPT-2-V1"
VERIFICATION_ID = "EXP-047-FORMAL-SEED-43-ATTEMPT-2-GATE-VERIFY-V1"
STAGE = "formal-seed-43-train-only-attempt-2"
SCRIPT_DIR = Path(__file__).resolve().parent
PROJECT_ROOT = SCRIPT_DIR.parents[2]
REPO_ROOT = PROJECT_ROOT.parents[1]
CONTRACT_PATH = SCRIPT_DIR / "preflight" / "exp-047-formal-seed-43-attempt-2-contract-v1.json"
...
def verify_runtime_and_data(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    runtime_path = resolve_project_path(contract["frozen_artifacts"]["runtime_seed_43"]["path"])
...
def verify_seed42_predecessors(contract: dict[str, Any]) -> dict[str, Any]:
    expected = {
        "seed_42_replay_verification": "EXP-047-SEED-42-REPLAY-VERIFY-V2",
        "seed_42_training_verification": "EXP-047-FORMAL-SEED-42-TRAIN-VERIFY-V2",
    }
    observed: dict[str, Any] = {}
    for key, verification_id in expected.items():
        ...
        if (
            ...
            raise ValueError(f"Invalid seed-42 predecessor evidence: {key}")
        observed[key] = artifact(path)
    return observed


def verify_attempt1_failure(contract: dict[str, Any]) -> dict[str, Any]:
    run_path = resolve_project_path(contract["frozen_artifacts"]["seed_43_attempt_1_failure"]["path"])
    log_path = resolve_project_path(contract["frozen_artifacts"]["seed_43_attempt_1_stdout"]["path"])
    run = load_json(run_path)
    if (
        run.get("experiment_id") != EXPERIMENT_ID
        or run.get("seed") != 43
        or run.get("stage") != "formal-seed-43-train-only"
        ...
        or run.get("error", {}).get("message") != "MLX-LM formal training exited with status -6"
    ):
        raise ValueError("Seed-43 attempt-1 failure evidence is invalid")
    text = log_path.read_text(encoding="utf-8")
    if "No Metal device available" not in text or "Iter " in text:
        raise ValueError("Seed-43 attempt-1 did not stop before the first training iteration")
    adapter_dir = resolve_project_path(contract["correction"]["attempt_1_adapter_dir"])
    if not adapter_dir.is_dir() or any(adapter_dir.iterdir()):
        raise ValueError("Seed-43 attempt-1 adapter directory is not empty")
    return {
        "adapter_files": 0,
        "failure": artifact(run_path),
        "first_training_iteration_reached": False,
...
def recompute(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "attempt_1_failure": verify_attempt1_failure(contract),
        "environment": verify_environment(contract),
        "model": verify_model(contract),
        "predecessors": verify_seed42_predecessors(contract),
...
def build_verification(contract: dict[str, Any], completed_at_utc: str) -> dict[str, Any]:
    return {
        ...
        "next_required_step": "Bind the corrected seed-43 attempt-2 authorization and execute outside the restricted Metal session; seed 44 and validation/test remain closed.",
