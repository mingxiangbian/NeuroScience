#!/usr/bin/env python3
"""Run the preregistered EXP-011 RoBERTa-base fine-tuning experiment."""
...
DEFAULT_CONFIG = SCRIPT_DIR / "configs" / "exp-011-roberta-base.json"
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "runs" / "exp-011-roberta-base-finetuning"
REQUIREMENTS_LOCK = SCRIPT_DIR / "requirements-lock.txt"
PROTOCOL_PATH = (
    SCRIPT_DIR
    / "protocols"
    / "exp-011-roberta-base-finetuning.md"
)
...
EXPECTED_PROTOCOL_SHA256 = (
    "c0e2d1f49b8e6f43381519a33c2ae4d2c6d0143546bf6825cc944130b881fd26"
)
...
def save_confusion_figure(
    ...
    axis.set_title(f"EXP-011 Seed {seed} Validation")
    figure.tight_layout()
...
def main() -> None:
    ...
    with stdout_path.open("a", encoding="utf-8") as stdout_file:
        ...
        try:
            log("EXP-011 starting; test access is prohibited")
            ...
            log(
                "EXP-011 completed: mean validation Macro-F1 "
            ...
            write_json(run_path, run_state)
            log(f"EXP-011 failed: {type(error).__name__}: {error}")
            raise
        finally:
