# EXP-011 Protocol: RoBERTa-Base Fine-Tuning Execution Retry

Registration date: 2026-07-30

This protocol is frozen before EXP-011 reads validation predictions or
...
- Tier: Major
- RQ: RQ-B2
- Parent experiment: EXP-010
- Retry reason: EXP-010 failed at its pre-data environment gate because the
  restricted execution process did not expose Apple MPS
- Stage: train-validation
- Dataset: TweetEval emotion
- Allowed splits: official train and validation only
- Prohibited split: test
- Frozen config:
  `configs/exp-011-roberta-base.json`
- Frozen config SHA-256:
  `15ae9b0d5adcae9dd98519ba84a5ae1207fd00f89fab8b485a0b89d5a555901a`
...
EXP-010 then stopped at its environment gate, before opening any dataset file,
because the restricted process reported MPS unavailable. Its source, failure
metadata, and log are preserved under
`runs/exp-010-roberta-base-finetuning/`.

EXP-011 changes no scientific condition. Data, model revision, tokenization,
hyperparameters, seeds, selection rules, metrics, comparison baseline, and
resource budget are identical to EXP-009 and EXP-010. Its runner is identical
to EXP-010 except for experiment-specific paths, identifiers, and frozen-file
hashes. It is launched on the same machine outside the restricted process so
the already required Apple MPS device is visible. Any functional correction
requires another experiment ID.
...
Negative outcome: if the threshold is not reached, EXP-011 still establishes
...
tuning EXP-011 after validation is read.
...
duplicates, and no exact train-validation text overlap. EXP-011 records these
...
There is no EXP-011 learning-rate, batch-size, epoch, class-weight, or
...
```text
mean(EXP-011 validation Macro-F1) - 0.6226779061 >= 0.005
```
...
```text
runs/exp-011-roberta-base-finetuning/
```
