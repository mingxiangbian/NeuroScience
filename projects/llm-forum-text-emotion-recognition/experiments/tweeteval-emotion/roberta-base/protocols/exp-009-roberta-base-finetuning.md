# EXP-009 Protocol: RoBERTa-Base Fine-Tuning

Registration date: 2026-07-30

This protocol is frozen before EXP-009 reads validation predictions or
...
Negative outcome: if the threshold is not reached, EXP-009 still establishes
...
tuning EXP-009 after validation is read.
...
duplicates, and no exact train-validation text overlap. EXP-009 records these
...
There is no EXP-009 learning-rate, batch-size, epoch, class-weight, or
...
```text
mean(EXP-009 validation Macro-F1) - 0.6226779061 >= 0.005
```
