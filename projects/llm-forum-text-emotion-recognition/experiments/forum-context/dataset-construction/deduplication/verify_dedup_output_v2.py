#!/usr/bin/env python3
"""Independently verify DATA-FCTX-DEDUP-V2 without emitting private text."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--reuse-embeddings-from", type=Path, required=True)
    parser.add_argument("--reuse-index-from", type=Path, required=True)
...
def verify(args: argparse.Namespace) -> dict[str, object]:
    embedding_worker = Path(args.script).with_name("embed_iac2_posts_v2.py")
    replay_worker = Path(__file__).with_name("replay_iac2_embeddings_v2.py")
    required = (
        args.input_db,
        args.db,
        args.embeddings,
    ...
    reuse_report = json.loads(args.reuse_report.read_text(encoding="utf-8"))
    reuse_manifest = json.loads(args.reuse_manifest.read_text(encoding="utf-8"))
    ...
    reuse_report_sha = sha256_file(args.reuse_report)
    reuse_manifest_sha = sha256_file(args.reuse_manifest)
    reuse_embeddings_sha = sha256_file(args.reuse_embeddings_from)
    reuse_index_sha = sha256_file(args.reuse_index_from)

    check(
        "pipeline status",
        report.get("pipeline_id") == PIPELINE_ID
        and report.get("status") == "completed_pending_independent_verification",
        "V2 pre-verification run completed",
    )
    check(
        "manifest status",
        manifest.get("pipeline_id") == PIPELINE_ID
        and manifest.get("artifact_status")
        == "generated_pending_independent_verification",
        "V2 artifact awaits this verification",
    )
    ...
    reuse = report.get("reuse_provenance")
    check("reuse provenance present", isinstance(reuse, dict), "V1 artifacts reused under hash control")
    if isinstance(reuse, dict):
        expected_reuse = {
            "source_pipeline_id": "DATA-FCTX-DEDUP-V1",
            "source_artifact_status": "failed_retrieval_quality_gate",
            "source_manifest_filename": args.reuse_manifest.name,
            "source_manifest_sha256": reuse_manifest_sha,
            "source_report_filename": args.reuse_report.name,
            "source_report_sha256": reuse_report_sha,
            "embedding_filename": args.reuse_embeddings_from.name,
            "embedding_sha256": reuse_embeddings_sha,
            "index_filename": args.reuse_index_from.name,
            "index_sha256": reuse_index_sha,
            "reused_components": ["post_embeddings", "HNSW_graph"],
            "reused_decisions_or_edges": False,
        }
        check("reuse provenance fields", reuse == expected_reuse, "hashes and component boundary")
        check("reuse provenance in manifest", manifest.get("reuse_provenance") == reuse, "report and manifest agree")
    check(
        "reuse source failed status",
        reuse_manifest.get("pipeline_id") == "DATA-FCTX-DEDUP-V1"
        and reuse_manifest.get("artifact_status") == "failed_retrieval_quality_gate"
        and reuse_report.get("pipeline_id") == "DATA-FCTX-DEDUP-V1"
        and reuse_report.get("status") == "failed_retrieval_quality_gate",
        "V1 remains failed evidence",
    )
    check(
        "reuse report-manifest hash",
        reuse_manifest["aggregate_report"]["sha256"] == reuse_report_sha,
        "SHA-256",
    )
    check(
        "reuse embedding hash",
        reuse_manifest["private_artifacts"]["embeddings"]["sha256"]
        == reuse_embeddings_sha,
        "SHA-256",
    )
    check(
        "reuse index hash",
        reuse_manifest["private_artifacts"]["index"]["sha256"] == reuse_index_sha,
        "SHA-256",
    )
    check(
        "reused embedding unchanged",
        embeddings_sha == reuse_embeddings_sha,
        "byte-identical frozen vectors",
    )
    check(
        "V2 index serialization updated",
        index_sha != reuse_index_sha,
        "efSearch metadata changed from V1",
    )
    check(
        "reuse source input and model",
        reuse_report["input"]["database_sha256"] == input_sha
        and reuse_manifest["input"]["database_sha256"] == input_sha
        and reuse_report["model"]["model_id"] == MODEL_ID
        and reuse_report["model"]["revision"] == MODEL_REVISION
        and reuse_report["model"]["selected_files_sha256"] == model_sha,
        "same data and embedding model",
    )
    check(
        "no failed-run decisions or edges reused",
        report["scope"].get("reused_failed_run_decisions_or_edges") is False
        and isinstance(reuse, dict)
        and reuse.get("reused_decisions_or_edges") is False,
        "V2 decisions regenerated",
    )
    ...
    public_violations.extend(public_payload_violations(reuse_report, "reuse_report"))
    public_violations.extend(public_payload_violations(reuse_manifest, "reuse_manifest"))
    ...
    try:
        ...
        for index, pair_hash in enumerate(pair_hashes):
            pair_groups[pair_hash].append(index)
        duplicate_groups = [
            members for members in pair_groups.values() if len(members) > 1
        ]
        duplicate_sizes = [len(members) for members in duplicate_groups]
        ...
        exact_resolution_errors = 0
        for members in duplicate_groups:
            finals = {int(canonical_indices[index]) for index in members}
            if len(finals) != 1:
                exact_resolution_errors += 1
                continue
            final = next(iter(finals))
            if final < 0 or str(decisions[final][3]) != "keep":
                exact_resolution_errors += 1
                continue
            final_matches_pair = pair_hashes[final] == pair_hashes[members[0]]
            for member in members:
                expected = (
                    "keep"
                    if final_matches_pair and member == final
                    else "drop_exact"
                    if final_matches_pair
                    else "drop_format_only"
                )
                if str(decisions[member][3]) != expected:
                    exact_resolution_errors += 1
        check(
            "exact group final resolution",
            exact_resolution_errors == 0,
            f"mismatches={exact_resolution_errors}",
        )
        ...
        check(
            "HNSW report configuration",
            report["retrieval"]["M"] == HNSW_M
            and report["retrieval"]["ef_construction"] == HNSW_EF_CONSTRUCTION
            and report["retrieval"]["ef_search"] == HNSW_EF_SEARCH
            and report["retrieval"].get("reused_graph") is True,
            "M=32, efConstruction=200, efSearch=768",
        )
        check(
            "HNSW persisted configuration",
            hnsw.hnsw.efConstruction == HNSW_EF_CONSTRUCTION
            and hnsw.hnsw.efSearch == HNSW_EF_SEARCH
            and hnsw.hnsw.nb_neighbors(1) == HNSW_M,
            "index parameters",
        )
    ...
    payload: dict[str, object] = {
        ...
        "inputs": {
            ...
            "reuse_manifest_filename": args.reuse_manifest.name,
            "reuse_manifest_sha256": reuse_manifest_sha,
            "reuse_report_filename": args.reuse_report.name,
            "reuse_report_sha256": reuse_report_sha,
            "reuse_embeddings_sha256": reuse_embeddings_sha,
            "reuse_index_sha256": reuse_index_sha,
        },
        "aggregate": {
            ...
            "exact_duplicate_clusters": int(report["exact_duplicates"]["clusters"]),
            "exact_duplicate_members": int(report["exact_duplicates"]["members"]),
