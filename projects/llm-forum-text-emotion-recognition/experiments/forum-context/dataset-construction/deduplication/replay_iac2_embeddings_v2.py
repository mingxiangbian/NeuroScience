#!/usr/bin/env python3
"""Replay a deterministic V2 embedding/token-count sample without FAISS."""
    ...
    try:
        ...
        sample_rows = heapq.nsmallest(
            ...
            key=lambda index: hashlib.sha256(
                str(post_rows[index][0]).encode("ascii")
            ).digest(),
        )
        texts: list[str] = []
        ...
        with torch.inference_mode():
            for start in range(0, len(texts), args.batch_size):
                ...
                token_count_errors += int(
                    np.count_nonzero(lengths != expected_lengths)
                )
