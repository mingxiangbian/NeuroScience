#!/usr/bin/env python3
"""Stream private IAC posts into local MiniLM embeddings without loading FAISS."""
...
def encode(args: argparse.Namespace) -> dict[str, object]:
    ...
    for path in (args.embeddings, args.token_counts, args.stats):
    ...
    try:
        ...
        with torch.inference_mode():
            while True:
                ...
                written = stop
                if written % 10000 == 0 or written == post_count:
                    elapsed = max(time.monotonic() - started, 1e-9)
                    print(
                        f"embedded posts: {written:,}/{post_count:,} "
