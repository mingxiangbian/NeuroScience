...
class Exp065LabelFreeProjectionTests(unittest.TestCase):
    ...
    def test_attempt2_source_aliases_are_safe_metadata(self) -> None:
        config = json.loads(RUNNER.DEFAULT_CONFIG.read_text(encoding="utf-8"))
        aliases = RUNNER.public_source_aliases(config["inputs"])
        self.assertEqual(
            set(aliases),
            {
                "validation_source", "m1_stack_source", "m1_selection_source",
                "m3_stack_source", "m3_selection_source",
            },
        )
        self.assertEqual(RUNNER.public_sensitive_paths({"source_aliases": aliases}), [])
        self.assertEqual(aliases, VERIFIER.expected_public_source_aliases(config["inputs"]))
