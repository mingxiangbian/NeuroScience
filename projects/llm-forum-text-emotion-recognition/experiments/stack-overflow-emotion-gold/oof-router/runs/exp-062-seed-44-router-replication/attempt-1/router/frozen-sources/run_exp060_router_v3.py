#!/usr/bin/env python3
"""Governed seed-44 launcher for the frozen EXP-060 router runner.

This module validates the v3/preflight release contract, temporarily adapts only
the two frozen governance constants, and calls the byte-pinned v2 runner once.
It does not alter any scientific policy or computation function.
"""
...
def _validate_process_runtime() -> None:
    if os.path.realpath(sys.executable) != os.path.realpath(EXPECTED_RUNTIME["python_executable"]):
        raise PermissionError("Formal runner is not using the frozen Python environment")
    observed = {
    ...
    if not _typed_equal(observed, {key: EXPECTED_RUNTIME[key] for key in observed}):
        raise PermissionError(f"Formal runner dependency version drift: {observed}")
...
def _validate_preflight(formal: dict[str, Any]) -> None:
    ...
    expected_preflight_paths = {
        ...
        "runner_launcher": Path(__file__).resolve(),
        "verifier_launcher": VERIFIER_LAUNCHER_PATH,
...
def _load_base_runner(config: dict[str, Any]) -> ModuleType:
    payload, _ = _secure_read(BASE_RUNNER_PATH, 0o644)
    if _sha256(payload) != BASE_RUNNER_SHA256:
        raise ValueError("Frozen base runner hash drift before import")
    if config["implementation"]["runner"]["sha256"] != BASE_RUNNER_SHA256:
        raise ValueError("Config does not bind the frozen base runner")
    module_name = "_exp060_router_base_v2_runner"
    module = ModuleType(module_name)
    module.__file__ = str(BASE_RUNNER_PATH)
    module.__package__ = ""
    code = compile(
        payload.decode("utf-8"), str(BASE_RUNNER_PATH), "exec", dont_inherit=True
    )
...
def _validate_post_state(config: dict[str, Any], stage: str) -> None:
    outputs = _expected_outputs()
    public_attempt = PROJECT_ROOT / outputs["public_attempt_dir"]
    ...
    if stage == "complete":
        expected_root.update({"verification.json", "VERIFICATION-SUMMARY.md", "router-complete.json"})
...
def execute(config_path: Path, stage: str, *, base_module: ModuleType | None = None) -> dict[str, Any]:
    ...
    base = base_module if base_module is not None else _load_base_runner(prevalidated)
    ...
    try:
        ...
        loaded = base.load_config(canonical)
        if loaded != prevalidated:
            raise ValueError("Frozen base runner changed the prevalidated config")
        result = (
            base.formal_run(canonical, loaded)
            if stage == "run"
            else base.complete_router_attempt(canonical, loaded)
        )
    finally:
    ...
    _validate_post_state(postvalidated, stage)
    outer_elapsed = time.monotonic() - outer_started
    outer_rss = _peak_rss_gb()
    if (
        outer_elapsed > float(FORMAL_RESOURCE_CONTRACT["formal_wall_seconds"])
        or outer_rss > float(FORMAL_RESOURCE_CONTRACT["formal_peak_memory_gb"])
    ):
        raise RuntimeError(
            f"Outer governed runner budget exceeded: wall={outer_elapsed}, rss={outer_rss}"
        )
    return result
