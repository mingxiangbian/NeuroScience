#!/usr/bin/env python3
"""Incident-003-aware governed launcher for the frozen EXP-060 v3 runner.

This is a thin consumer.  It validates the append-only recovery lineage, loads
the byte-pinned v3 launcher from its Attempt-1 frozen copy, patches exactly the
schema and launch-contract callback, and invokes v3 once.  It does not contain
or change router science.
"""
...
V3_LIVE_PATH = SCRIPT_DIR / "run_exp060_router_v3.py"
V3_VERIFIER_PATH = SCRIPT_DIR / "verify_exp060_router_v3.py"
V3_TESTS_PATH = SCRIPT_DIR / "tests/test_exp060_router_v3.py"
V4_RUNNER_PATH = Path(__file__).resolve()
...
V3_FROZEN_PATH = ATTEMPT1_FROZEN / "run_exp060_router_v3.py"
...
V3_BYTES = 65859
V3_SHA256 = "a443c6a7ec7c35fc91ec4acf879f5ac7abb0d654e37fd72f43add9bdfba1a4bd"
V3_VERIFIER_SHA256 = "38da93d352de49121d5035d11fa73fe4f7b349206227adc46db5be814db97651"
...
RECOVERY_FROZEN_EXPECTED = {
    ...
    "protocol": (
        "exp-062-exp060-router-preflight-verification-attempt-2-incident-003.md",
        11129,
        "b56001a7680a0ebca8dac27f3f2ea7a5cdbbc126a7526f40d1a3821591e984f3",
    ),
    "verifier": (
        "verify_exp060_router_preflight_attempt2.py",
        72377,
        "b0858368ab2eb5c4c1428c497f36a98735fb59f94c286f090ae82bb55baa3528",
    ),
    "tests": (
        "test_exp060_router_preflight_attempt2.py",
        43509,
...
RECOVERY_ATTEMPT1_KEYS = {
    "config", "run", "contract", "frozen_verifier", "failure_seal",
}
RECOVERY_ATTEMPT2_KEYS = {
    "incident_config", "claim", "verification", "summary", "terminal",
    "frozen_sources",
}
...
PASSED_KEYS = {
    "schema_version", "incident_id", "verification_attempt", "experiment_id",
    "replication_parent_experiment_id", "run_id", "attempt_id", "model_seed",
    "seed_contract", "rq_id", "scope", "status", "verified_at_utc",
    "patch_ledger", "original_artifacts", "physical_invariants",
    "virtual_namespace", "captured_candidate", "checks", "passed_count",
...
ORIGINAL_VERIFICATION_KEYS = {
    ...
    "run_id", "attempt_id", "model_seed", "seed_contract", "rq_id", "scope",
    "verified_at_utc", "status", "passed_count", "failed_count", "checks",
    "independent", "runner_imported", "execution_claims", "verified_artifacts",
    "resources", "claim_boundary",
}
...
EXECUTION_CLAIM_KEYS = {
    "array_values_loaded", "real_features_constructed",
    "real_router_targets_constructed", "scaler_or_router_fitted",
    "router_metrics_computed", "bootstrap_computed", "validation_accessed",
    "test_accessed", "raw_text_accessed", "model_loaded_or_forward_run",
    "formal_router_training", "formal_result_computation",
}
...
def _secure_read(path: Path, mode: int) -> tuple[bytes, os.stat_result]:
    ...
    if (
        _stat_signature(after_fd) != _stat_signature(opened)
...
def _held_v3_bytes(live_path: Path, frozen_path: Path) -> bytes:
    ...
    try:
        ...
        if (
            ...
            raise ValueError("Frozen/live v3 runner bytes drift")
        return payloads[1]
    finally:
...
def _require_record(record: Any, mode: int) -> tuple[Path, bytes]:
    ...
    if not isinstance(record["path"], str) or not record["path"]:
        raise ValueError("Artifact record path type drift")
    relative = Path(record["path"])
    if relative.is_absolute() or ".." in relative.parts:
        raise ValueError("Artifact record path is not project-relative")
    if not _typed_int(record["bytes"]) or record["bytes"] < 0:
        raise ValueError("Artifact record byte type drift")
    if (
        not isinstance(record["sha256"], str)
        or len(record["sha256"]) != 64
        or any(character not in "0123456789abcdef" for character in record["sha256"])
    ):
        raise ValueError("Artifact record SHA-256 type drift")
...
def _json_record(record: Any, mode: int = 0o644) -> tuple[Path, dict[str, Any]]:
...
def _fixed_record(name: str) -> tuple[dict[str, Any], bytes]:
    ...
    return {
        "path": _display(path), "bytes": expected_bytes, "sha256": expected_sha,
    }, payload


def _directory(path: Path, mode: int, names: set[str]) -> None:
    metadata = os.lstat(path)
    if (
        ...
        raise PermissionError(f"Governed directory mode/type drift: {path}")
    observed = {entry.name for entry in os.scandir(path)}
    if observed != names:
        raise ValueError(f"Governed directory inventory drift: {path}")


def _snapshot_directory(path: Path, names: set[str]) -> dict[str, Any]:
    _directory(path, 0o755, names)
...
def _recovery_terminal_state_snapshot(incident_config: dict[str, Any]) -> dict[str, Any]:
    _, run_payload = _fixed_record("attempt1_run")
    _, config_payload = _fixed_record("attempt1_config")
    ...
    return {
        ...
        # This is a reconstruction of the terminal historical state. Current
        # scope outputs are validated separately by _stage_state.
...
def _validate_recovery_transition_claim(config: dict[str, Any]) -> None:
    ...
    if (
        ...
        or claim.get("v3_launcher") != _record(V3_LIVE_PATH)
...
def _exact_checks(
    ...
    if (
        ...
        or any(
            ...
            or (null_details and row["detail"] is not None)
            for row in checks
        )
...
def _validate_recovery(config: dict[str, Any], v3: ModuleType) -> None:
    ...
    if not isinstance(recovery, dict) or set(recovery) != RECOVERY_KEYS:
        raise ValueError("Incident 003 recovery inventory drift")
    if (
        recovery.get("incident_id") != INCIDENT_ID
        or recovery.get("canonical_attempt_1_sidecars_absent") is not True
        or recovery.get("formal_gate_authorized") is not False
    ):
        raise ValueError("Incident 003 recovery identity/boundary drift")
    attempt1 = recovery.get("attempt_1")
    attempt2 = recovery.get("attempt_2")
    if not isinstance(attempt1, dict) or set(attempt1) != RECOVERY_ATTEMPT1_KEYS:
        raise ValueError("Attempt-1 recovery inventory drift")
    if not isinstance(attempt2, dict) or set(attempt2) != RECOVERY_ATTEMPT2_KEYS:
        raise ValueError("Attempt-2 recovery inventory drift")

    fixed: dict[str, tuple[dict[str, Any], bytes]] = {
        name: _fixed_record(name) for name in FIXED_ARTIFACTS
    }
    expected_attempt1 = {
        "config": fixed["attempt1_config"][0],
        "run": fixed["attempt1_run"][0],
    ...
    if not _typed_equal(attempt1, expected_attempt1):
        raise ValueError("Attempt-1 direct record lineage drift")

    frozen_records: dict[str, dict[str, Any]] = {}
    _directory(ATTEMPT2_FROZEN, 0o755, {row[0] for row in RECOVERY_FROZEN_EXPECTED.values()})
    for name, (basename, expected_bytes, expected_sha) in RECOVERY_FROZEN_EXPECTED.items():
        path = ATTEMPT2_FROZEN / basename
        payload, metadata = _secure_read(path, 0o644)
        if metadata.st_size != expected_bytes or _sha256(payload) != expected_sha:
            raise ValueError(f"Incident 003 frozen source drift: {name}")
        frozen_records[name] = {
            "path": _display(path), "bytes": expected_bytes, "sha256": expected_sha,
        }
    expected_attempt2 = {
        "incident_config": fixed["incident_config"][0],
        "claim": fixed["claim"][0],
        "verification": fixed["attempt2_verification"][0],
        "summary": fixed["attempt2_summary"][0],
        "terminal": fixed["attempt2_terminal"][0],
        "frozen_sources": frozen_records,
    }
    if not _typed_equal(attempt2, expected_attempt2):
        raise ValueError("Attempt-2 direct record lineage drift")
    ...
    for forbidden in (
        ATTEMPT2_ROOT / "attempt-2-failure.json",
        ATTEMPT2_ROOT / "FAILURE-SUMMARY.md",
        CANONICAL_ATTEMPT1_VERIFICATION,
        CANONICAL_ATTEMPT1_SUMMARY,
        OLD_FORMAL_CONFIG,
        SELECTION_PATH,
    ):
    ...
    incident_config = json.loads(fixed["incident_config"][1].decode("utf-8"))
    claim = json.loads(fixed["claim"][1].decode("utf-8"))
    failure = json.loads(fixed["attempt1_failure"][1].decode("utf-8"))
    passed = json.loads(fixed["attempt2_verification"][1].decode("utf-8"))
    terminal = json.loads(fixed["attempt2_terminal"][1].decode("utf-8"))
    summary_bytes = fixed["attempt2_summary"][1]
    live_recovery = {
    ...
    if (
        ...
        raise ValueError("Incident 003 claim drift")
    claimed_at = _canonical_utc(claim.get("claimed_at_utc"))

    if (
        not isinstance(failure, dict)
        or set(failure) != ATTEMPT1_FAILURE_KEYS
        or not _identity(failure, attempt=1)
    ...
    if (
        ...
        raise ValueError("Incident 003 Passed envelope drift")
    _exact_checks(passed, RECOVERY_CHECK_NAMES)
    verified_at = _canonical_utc(passed.get("verified_at_utc"))
    if verified_at < claimed_at:
        raise ValueError("Incident 003 timestamp chronology drift")
    if not _typed_equal(passed.get("patch_ledger"), {
        ...
        raise ValueError("Incident 003 patch ledger drift")
    if not _typed_equal(passed.get("virtual_namespace"), {
        ...
        "unverified_tree_calls": 1,
        "verified_tree_calls": 1,
        "writer_calls": 2,
    }):
        raise ValueError("Incident 003 virtual namespace drift")
    if not _typed_equal(passed.get("physical_invariants"), {
        "after_sha256": TERMINAL_STATE_SHA256,
        "before_sha256": TERMINAL_STATE_SHA256,
        "canonical_sidecars_absent": True,
        "downstream_absent": True,
        "unchanged": True,
    }):
        raise ValueError("Incident 003 physical invariant drift")
    _validate_recovery_terminal_phase(config, incident_config)
    if not _resources(passed.get("resources"), 300, 2.0):
        raise ValueError("Incident 003 Passed resource drift")

    original = passed.get("original_artifacts")
    if not isinstance(original, dict) or set(original) != ORIGINAL_ARTIFACT_KEYS:
        raise ValueError("Incident 003 original-artifact inventory drift")
    if (
        original.get("config") != expected_attempt1["config"]
        ...
        raise ValueError("Incident 003 original-artifact direct binding drift")
    for record in original.get("upstreams", {}).values():
    ...
    _directory(ATTEMPT1_FROZEN, 0o755, frozen_names)
    if (
        incident_config.get("predecessor", {}).get("source_artifacts")
        != original.get("source_artifacts")
        or incident_config.get("predecessor", {}).get("upstreams")
        != original.get("upstreams")
        or incident_config.get("predecessor", {}).get("input") != original.get("input")
    ):
        raise ValueError("Incident config/predecessor lineage drift")
    ...
    if (
        ...
        or set(candidate) != ORIGINAL_VERIFICATION_KEYS
    ...
    _exact_checks(candidate, ORIGINAL_CHECK_NAMES, null_details=False)
    ...
    if (
        ...
        raise ValueError("Captured no-result claims/resources drift")
    expected_verified = {
        "config": original["config"], "run": original["run"],
        "contract": original["contract"], "input": original["input"],
        "upstreams": original["upstreams"],
        "frozen_sources": original["source_artifacts"],
        "formal_sources": {
    ...
    if not _typed_equal(candidate.get("verified_artifacts"), expected_verified):
        raise ValueError("Captured original verified-artifact lineage drift")
    candidate_bytes = (
    ...
    if (
        ...
        raise ValueError("Captured candidate byte/digest drift")

    if summary_bytes != _passed_summary(passed):
        raise ValueError("Incident 003 deterministic summary drift")
    if (
        ...
        raise ValueError("Incident 003 terminal seal drift")
    completed_at = _canonical_utc(terminal.get("completed_at_utc"))
    if completed_at < verified_at:
        raise ValueError("Incident 003 terminal chronology drift")
    if any(_privacy(value) for value in (claim, failure, passed, terminal)):
        raise ValueError("Incident 003 public privacy drift")

    if config["prerequisites"]["exp060_preflight_verification"] != fixed["attempt2_verification"][0]:
        raise ValueError("Formal preflight prerequisite is not Attempt-2 Passed verification")
    ...
    for name in old_keys:
        ...
        if live["bytes"] != frozen["bytes"] or live["sha256"] != frozen["sha256"]:
            raise ValueError(f"Formal old-seven implementation lineage drift: {name}")
...
def _expected_commands(config_path: Path, python_executable: str) -> dict[str, list[str]]:
    config_argument = str(config_path.relative_to(REPO_ROOT.resolve()))
    runner_argument = str(V4_RUNNER_PATH.relative_to(REPO_ROOT.resolve()))
    verifier_argument = str(V4_VERIFIER_PATH.relative_to(REPO_ROOT.resolve()))
    return {
        "run": [python_executable, runner_argument, "--config", config_argument, "--stage", "run"],
        "verify_final": [python_executable, verifier_argument, "--config", config_argument, "--scope", "final"],
        "complete": [python_executable, runner_argument, "--config", config_argument, "--stage", "complete"],
        "verify_completion": [python_executable, verifier_argument, "--config", config_argument, "--scope", "completion"],
    }
...
def validate_launch_contract(config_path: Path, v3: ModuleType) -> tuple[Path, dict[str, Any]]:
    ...
    expected_top = set(v3.FORMAL_TOP_KEYS) | {"preflight_recovery"}
    ...
    if (
        ...
        raise ValueError("Formal v4 identity/path drift")
    if not _typed_equal(config.get("resources"), v3.FORMAL_RESOURCE_CONTRACT):
        raise ValueError("Formal v4 resource contract drift")
    if os.path.lexists(SELECTION_PATH):
        raise FileExistsError("Final pipeline selection already exists")
    ...
    if not isinstance(python_executable, str) or not Path(python_executable).is_absolute():
        raise ValueError("Formal v4 Python executable must be absolute")
    expected_commands = _expected_commands(path, python_executable)
    ...
    expected_paths = {
        ...
        "runner_launcher": V3_LIVE_PATH,
        "verifier_launcher": V3_VERIFIER_PATH,
        "launcher_tests": V3_TESTS_PATH,
        "incident_protocol": INCIDENT_PROTOCOL_PATH,
    ...
    if len(basenames) != len(expected_paths):
        raise ValueError("Formal v4 implementation basenames are not unique")
    frozen_hashes = {
        "runner": v3.BASE_RUNNER_SHA256,
        "verifier": v3.BASE_VERIFIER_SHA256,
        "tests": v3.BASE_TESTS_SHA256,
        "runner_launcher": V3_SHA256,
        "verifier_launcher": V3_VERIFIER_SHA256,
        "launcher_tests": V3_TESTS_SHA256,
    }
    for name, expected_hash in frozen_hashes.items():
        if implementation[name]["sha256"] != expected_hash:
            raise ValueError(f"Frozen base/v3 implementation hash drift: {name}")
    ...
    expected_paired = PRIVATE_ATTEMPT / "paired-oof.npz"
    if paired != expected_paired.resolve():
    ...
    exact_sections = {
        "provenance": v3.EXPECTED_PROVENANCE,
        "path_bases": v3.EXPECTED_PATH_BASES,
        "artifact_contract": v3.FORMAL_ARTIFACT_CONTRACT,
        "runtime": v3.EXPECTED_RUNTIME,
        "data": v3.EXPECTED_DATA,
        "cross_fitting": v3.EXPECTED_CROSS_FITTING,
        "thresholds": v3.EXPECTED_THRESHOLDS,
        "target_contract": v3.EXPECTED_TARGET_CONTRACT,
        "feature_contract": v3.EXPECTED_FEATURE_CONTRACT,
        "router": v3.EXPECTED_ROUTER,
        "call_rates": v3.EXPECTED_CALL_RATES,
        "gate": v3.EXPECTED_GATE,
        "risk_coverage": v3.EXPECTED_RISK_COVERAGE,
        "bootstrap": v3.EXPECTED_BOOTSTRAP,
    }
...
def _load_verified_v3() -> ModuleType:
    frozen_payload = _held_v3_bytes(V3_LIVE_PATH, V3_FROZEN_PATH)
    module = ModuleType("_exp060_router_v4_incident003_frozen_v3_runner")
    module.__file__ = str(V3_LIVE_PATH)
    module.__package__ = ""
    code = compile(frozen_payload.decode("utf-8"), str(V3_LIVE_PATH), "exec", dont_inherit=True)
    exec(code, module.__dict__)
    if module.FORMAL_CONFIG_SCHEMA != V3_CONFIG_SCHEMA or not callable(module.validate_launch_contract):
        raise ValueError("Frozen v3 runner interface drift")
    return module
...
def _immutable_snapshot(config_path: Path, config: dict[str, Any]) -> dict[str, Any]:
    config_payload, config_metadata = _secure_read(config_path, 0o644)
    records: list[tuple[str, dict[str, Any], int]] = []
    records.extend((f"implementation.{name}", record, 0o644) for name, record in config["implementation"].items())
    records.extend((f"prerequisites.{name}", record, 0o644) for name, record in config["prerequisites"].items())
    records.append(("input.paired_oof", config["input"]["paired_oof"], 0o600))
    attempt1 = config["preflight_recovery"]["attempt_1"]
    attempt2 = config["preflight_recovery"]["attempt_2"]
    records.extend((f"recovery.attempt1.{name}", record, 0o644) for name, record in attempt1.items())
    for name, record in attempt2.items():
        if name == "frozen_sources":
            records.extend((f"recovery.frozen.{child}", child_record, 0o644) for child, child_record in record.items())
        else:
            records.append((f"recovery.attempt2.{name}", record, 0o644))
    observed: dict[str, Any] = {
        "config": (_stat_signature(config_metadata), _sha256(config_payload))
    }
    for name, record, mode in records:
        path, payload = _require_record(record, mode)
        metadata = os.lstat(path)
        observed[name] = (record, _stat_signature(metadata), _sha256(payload))
    observed["pyc"] = _pyc_snapshot()
    observed["canonical_absent"] = (
        not os.path.lexists(CANONICAL_ATTEMPT1_VERIFICATION),
        not os.path.lexists(CANONICAL_ATTEMPT1_SUMMARY),
...
def _stage_state(operation: str, *, before: bool) -> tuple[bool, ...]:
    public_router = PUBLIC_ATTEMPT / "router"
    private_router = PRIVATE_ATTEMPT / "router"
    ...
    values = tuple(os.path.lexists(path) for path in (public_router, private_router, verification, summary, completion, SELECTION_PATH))
    expected = {
        ("run", True): (False, False, False, False, False, False),
        ("run", False): (True, True, False, False, False, False),
        ("complete", True): (True, True, True, True, False, False),
        ("complete", False): (True, True, True, True, True, False),
    }[(operation, before)]
    if values != expected:
        raise ValueError(f"Formal v4 runner stage-state drift for {operation}: {values}")
    return values


def _peak_rss_gb() -> float:
    unit = 1e9 if platform.system() == "Darwin" else 1e9 / 1024
    return max(
...
def _stage_v3_path(scope: str) -> Path:
    return V3_LIVE_PATH if scope in {"run", "complete"} else V3_VERIFIER_PATH
...
def _governance_precheck(scope: str, config_path: Path, config: dict[str, Any]) -> None:
    ...
    _directory(GOVERNANCE_DIR, 0o755, expected)
    for previous in STAGE_ORDER[:index]:
...
def execute(config_path: Path, stage: str) -> dict[str, Any]:
    if stage not in {"run", "complete"}:
        raise ValueError("Unexpected v4 runner stage")
    ...
    _stage_state(stage, before=True)
    _governance_precheck(stage, canonical, config)
    claim_path = _claim_scope(stage, canonical, config)
    ...
    try:
        try:
            ...
            execute_calls += 1
            result = v3.execute(canonical, stage)
        finally:
        ...
        if (
            ...
            raise RuntimeError("v4 exact-two patch/call/restore contract drift")
        _validate_scope_result(stage, result)
        after_snapshot = _immutable_snapshot(canonical, config)
        if after_snapshot != before_snapshot:
            raise RuntimeError("Immutable Incident/upstream/private state changed during formal scope")
        _stage_state(stage, before=False)
        resources = _resource_value(started)
        if not _resources(resources, 1800, 4.0):
            raise RuntimeError(
                f"v4 outer runner budget exceeded: wall={resources['wall_seconds']}, "
                f"rss={resources['peak_process_rss_gb']}"
            )
        _seal_scope_success(
            stage, canonical, config, claim_path, before_snapshot, after_snapshot, resources
        )
        _governance_manifest_through(stage)
        _validate_completed_stage(stage, canonical, config)
        terminal_resources = _resource_value(started)
        if not _resources(terminal_resources, 1800, 4.0):
            raise RuntimeError("v4 runner terminal publication exceeded its outer budget")
        return result
    except Exception as error:
        try:
            _seal_scope_failure(stage, claim_path, error, started)
        except Exception as seal_error:
            raise RuntimeError("v4 runner failed and its failure seal could not be written") from seal_error
        raise
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--stage", choices=("run", "complete"), required=True)
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    result = execute(args.config, args.stage)
    if result.get("status") != _expected_result_status(args.stage):
        raise SystemExit(1)
    print(json.dumps({"status": result.get("status"), "stage": args.stage}, sort_keys=True))
