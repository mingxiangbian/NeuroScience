...
CONFIG_SCHEMA = "exp-066-runtime-parity-config-v2"
...
DEFAULT_CONFIG = PROJECT_ROOT / BASE / "configs/exp-066-headless-runtime-parity-attempt-2.json"
...
def load_config(config_path: Path) -> tuple[dict[str, Any], dict[str, Path]]:
    ...
    if set(recovery) != {"prior_claim", "prior_run", "reason", "method_changed", "data_changed"}:
        raise ValueError("EXP-066 verifier recovery contract drift")
    ...
    if (
        ...
        or recovery["reason"] != "m3_runtime_probability_replay_incompatibility"
        or recovery["method_changed"] is not False
        or recovery["data_changed"] is not False
    ):
        raise ValueError("EXP-066 verifier prior failed attempt lineage drift")
...
def verify(config_path: Path) -> dict[str, Any]:
    ...
    if run.get("recovery") != config.get("recovery"):
        raise ValueError("EXP-066 public recovery lineage drift")
    ...
    checks = [
        ...
        "exp065_prerequisite", "failed_attempt_lineage", "output_modes", "append_only_terminal_absence",
