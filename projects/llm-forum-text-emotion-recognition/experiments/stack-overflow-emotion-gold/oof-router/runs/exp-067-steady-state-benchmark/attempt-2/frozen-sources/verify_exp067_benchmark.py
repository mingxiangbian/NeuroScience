...
def load_config(path: Path) -> tuple[dict[str, Any], dict[str, Path]]:
    ...
    if set(recovery) != {"prior_claim", "prior_run", "reason", "method_changed", "data_changed"}:
        raise ValueError("EXP-067 verifier recovery contract drift")
    sources["prior_claim"] = require_record(recovery["prior_claim"], 0o644)
    sources["prior_run"] = require_record(recovery["prior_run"], 0o644)
    prior_claim = json.loads(sources["prior_claim"].read_text())
    prior_run = json.loads(sources["prior_run"].read_text())
    if (
        prior_claim.get("status") != "Claimed"
        or prior_run.get("status") != "Failed"
        or recovery["reason"] != "incomplete_shape_warmup_rss_growth_gate"
        or recovery["method_changed"] is not False
        or recovery["data_changed"] is not False
    ):
        raise ValueError("EXP-067 verifier prior failed attempt lineage drift")
...
def memory_gate(arrays: dict[str, np.ndarray]) -> dict[str, Any]:
    ...
    if (
        len(idle_indices) < 2
        or len(timed_indices) < 2
        or len(warm_indices) < 3
        or len(post_indices) < 3
    ):
    ...
    post_warm_rss = int(np.median(arrays["rss_bytes"][warm_indices[-3:]]))
    post_run_rss = int(np.median(arrays["rss_bytes"][post_indices[-3:]]))
...
def load_worker(rep: int, mode: str) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray], dict[str, Any]]:
    ...
    if (
        ...
        raise ValueError("EXP-067 worker recovery warmup contract drift")
...
def verify(config_path: Path) -> dict[str, Any]:
    ...
    if run.get("recovery") != config.get("recovery"):
        raise ValueError("EXP-067 public recovery lineage drift")
    ...
    checks = [
        "config_environment", "prerequisites", "failed_attempt_lineage", "output_modes", "append_only_terminal_absence",
