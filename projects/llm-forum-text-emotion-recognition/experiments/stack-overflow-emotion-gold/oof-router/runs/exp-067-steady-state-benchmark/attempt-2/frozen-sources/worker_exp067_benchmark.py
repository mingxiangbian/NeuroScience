...
CONFIG_SCHEMA = "exp-067-steady-state-benchmark-config-v2"
MODES = ("B0", "B1", "B2")
TIMING_SCHEMA = {
...
DEFAULT_CONFIG = PROJECT_ROOT / BASE / "configs/exp-067-steady-state-benchmark-attempt-2.json"
...
def m3_parts(
    backend: runtime_core.MlxM3Backend, text: str
    ...
    inference_ns = time.perf_counter_ns() - started
    return token_ns, inference_ns, np.ascontiguousarray(probabilities), len(ids)


def warm(mode: str, bundle: runtime_core.RouterBundle, m1: Any, m3: Any) -> None:
    observed_m1: list[int] = []
    observed_m3: list[int] = []
    for text in WARMUP_TEXTS:
        if mode in ("B0", "B2"):
            _, _, probability, length = m1_parts(m1, text)
            observed_m1.append(length)
            if mode == "B2":
                feature = runtime_core.build_features(probability, bundle.m1_threshold, len(text), length)
                bundle.route(feature)
        if mode in ("B1", "B2"):
            _, _, _, length = m3_parts(m3, text)
            observed_m3.append(length)
    if mode in ("B0", "B2") and max(observed_m1) != 256:
        raise RuntimeError("EXP-067 M1 max-length warmup coverage failed")
    if mode in ("B1", "B2") and max(observed_m3) != 384:
        raise RuntimeError("EXP-067 M3 max-length warmup coverage failed")
...
def timed_row(
    ...
    if mode == "B2":
        ...
        if route:
            token, m3_ns, m3_probability, _ = m3_parts(m3, text)
            tokenize_ns += token
    elif mode == "B1":
        token, m3_ns, m3_probability, _ = m3_parts(m3, text)
        tokenize_ns += token
...
def memory_gate(samples: list[dict[str, Any]]) -> dict[str, Any]:
    idle = [row for row in samples if row["phase"] == 0]
    timed = [row for row in samples if row["phase"] == 3]
    warm = [row for row in samples if row["phase"] == 2]
    post = [row for row in samples if row["phase"] == 4]
    if len(idle) < 2 or len(timed) < 2 or len(warm) < 3 or len(post) < 3:
    ...
    post_warm_rss = int(np.median([row["rss_bytes"] for row in warm[-3:]]))
    post_run_rss = int(np.median([row["rss_bytes"] for row in post[-3:]]))
    rss_retained = post_run_rss <= 1.10 * post_warm_rss
    return {
...
def run_worker(config: dict[str, Any], mode: str, repetition: int, output_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        gate = memory_gate(sampler.samples)
        ...
        manifest = {
            ...
            "warmup": {
                "synthetic_rows": 2,
                "maximum_sequence_lengths_covered": True,
                "resident_stabilization_seconds": 3,
