...
def load_config(path: Path) -> tuple[dict[str, Any], dict[str, Path]]:
    ...
    if set(recovery) != {"prior_claim", "prior_run", "reason", "method_changed", "data_changed"}:
        raise ValueError("EXP-067 recovery contract drift")
    ...
    if (
        ...
        raise ValueError("EXP-067 prior failed attempt lineage drift")
...
def execute(config_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        if smoke.get("status") != "Passed" or smoke.get("attempt_id") != ATTEMPT_ID:
