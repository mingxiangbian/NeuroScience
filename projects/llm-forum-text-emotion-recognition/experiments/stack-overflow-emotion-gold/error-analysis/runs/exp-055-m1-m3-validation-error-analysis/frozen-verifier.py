#!/usr/bin/env python3
"""Independent verifier for EXP-055. This module does not import the analyzer."""
...
def main() -> None:
    ...
    check("report validation boundary", "validation only" in report)
    check("report oracle boundary", "not deployable" in report)
    check("report qualitative boundary", "not dataset-wide prevalence" in report)
    check("report test boundary", "access test" in report and "did not" in report)
    check("report mechanism boundary", "internal emotion mechanism" in report)

    failed = [item["name"] for item in checks if not item["passed"]]
    payload = {
        "schema_version": "exp-055-m1-m3-error-analysis-verification-v1",
