#!/usr/bin/env python3
"""Independently verify EXP-053 seed-42 Classification LoRA train+validation."""
...
def verify(config_path: Path, run_dir: Path) -> dict[str, Any]:
    ...
    check("resource_verifier_102", resource_verify.get("status") == "Passed" and resource_verify.get("checks_passed") == 102 and resource_verify.get("failed_checks") == [])
    check("m2_seed42_verified_70", m2_verify.get("status") == "Passed" and m2_verify.get("check_count") == 70 and m2_verify.get("failed_checks") == [])
    check("m2_aggregate_verified_85", aggregate_verify.get("status") == "Passed" and aggregate_verify.get("check_count") == 85 and aggregate_verify.get("failed_checks") == [])
