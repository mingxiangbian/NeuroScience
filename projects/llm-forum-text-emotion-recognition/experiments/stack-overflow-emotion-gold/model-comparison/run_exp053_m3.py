#!/usr/bin/env python3
"""Run the authorized EXP-053 seed-42 Classification LoRA train+validation gate."""
...
def load_config(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    expected = {
        "seeds": [42], "splits": ["train", "validation"],
        "validation_access": True, "test_access": False,
        "formal_training": True, "seeds_43_44_authorized": False,
    ...
    if config.get("formal_execution_authorized") is not True:
        raise PermissionError("Formal EXP-053 seed 42 is not authorized")
...
def validate_prerequisites(config: dict[str, Any], shared: dict[str, Any]) -> dict[str, Any]:
    ...
    if m2_run.get("status") != "Completed":
        raise RuntimeError("Matched EXP-052 seed 42 did not complete")
    require_passed_verification(m2_verify, 70, "Matched EXP-052 seed 42")
...
def initialize_model(shared: dict[str, Any], config: dict[str, Any], train_rows: Sequence[dict[str, Any]], validation_rows: Sequence[dict[str, Any]], logger: logging.Logger) -> dict[str, Any]:
    ...
    if head_initial != config["execution"]["expected_head_initial_sha256"]:
        raise ValueError("Seed-42 head initialization does not match M2")
...
def run(config_path: Path, run_dir: Path) -> dict[str, Any]:
    ...
    for epoch, order in enumerate(batch_orders, start=1):
        ...
        history.append({
            "seed": 42, "epoch": epoch, "optimizer_steps": global_step,
    ...
    bootstrap = {
        "fixed_0.5": bootstrap_summary(validation_gold, fixed_predictions, component_ids, 42, replicates, namespace, "fixed-0.5"),
        "shared_threshold": bootstrap_summary(validation_gold, shared_predictions, component_ids, 42, replicates, namespace, f"shared-{selected_threshold:.2f}"),
    }
    ...
    paired = {
        ...
        "fixed_0.5": paired_bootstrap(validation_gold, fixed_predictions, m2_fixed_predictions, component_ids, 42, replicates, namespace, "fixed-0.5"),
        "shared_threshold": paired_bootstrap(validation_gold, shared_predictions, m2_shared_predictions, component_ids, 42, replicates, namespace, f"M3-{selected_threshold:.2f}_M2-{m2_run['threshold_selection']['selected_threshold']:.2f}"),
        "metric_deltas": {
    ...
    atomic_json(private_manifest_path, {
        "experiment_id": EXPERIMENT_ID, "stage": STAGE, "seed": 42,
    ...
    if time.perf_counter() - started > float(config["execution"]["maximum_total_wall_hours"]) * 3600:
        raise TimeoutError("Formal M3 exceeded frozen wall-time budget")
    logger.info("Formal seed 42 completed; independent verification is required")
    ...
    run_record = {
        ...
        "training": {
            "seed": 42, "epochs": int(spec["epochs"]), "batch_size": int(spec["batch_size"]),
        ...
        "warnings": [
            "This is one seed and does not establish the M3 family result.",
    ...
    report = "\n".join([
        "# EXP-053 M3 Classification LoRA Seed 42", "",
        ...
        "This is a single-seed validation result. Seeds 43/44, EXP-054 and test remain sealed.", "",
    ])
