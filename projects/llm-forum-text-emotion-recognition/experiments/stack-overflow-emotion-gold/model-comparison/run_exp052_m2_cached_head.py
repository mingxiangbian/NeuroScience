...
AUTHORIZED_CACHED_HEAD_SEEDS = (43, 44)


def preflight_stage(seed: int) -> str:
    if seed not in AUTHORIZED_CACHED_HEAD_SEEDS:
        raise PermissionError(f"Cached-head seed is not registered: {seed}")
    return f"seed-{seed}-cache-reuse-preflight"


def formal_stage(seed: int) -> str:
    if seed not in AUTHORIZED_CACHED_HEAD_SEEDS:
        raise PermissionError(f"Cached-head seed is not registered: {seed}")
    return f"seed-{seed}-cache-reuse-train-validation"
...
def load_config(path: Path, seed: int) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    expected_preflight = preflight_stage(seed)
    expected_formal = formal_stage(seed)
    if config.get("stage") not in (expected_preflight, expected_formal):
        raise ValueError("Unexpected EXP-052 cached-head stage")
    authorization = config["authorization"]
    if authorization.get("seeds") != [seed]:
        raise PermissionError(f"This contract must authorize exactly seed {seed}")
    ...
    if authorization.get("seed_44_authorized") is not (seed == 44):
        raise PermissionError("Seed-44 authorization flag does not match the seed")
    if authorization.get("exp_053_054_authorized") is not False:
        raise PermissionError("M3 and M4 remain sealed")
    if authorization.get("three_seed_aggregate_authorized", False) is not False:
        raise PermissionError("The three-seed M2 aggregate remains separately sealed")
    formal = config["stage"] == expected_formal
    ...
    verify_prior_seed_gate(config, seed)
    return config, shared


def verify_prior_seed_gate(config: dict[str, Any], seed: int) -> None:
    gate = config.get("prior_seed_gate")
    if seed == 43:
        if gate is not None:
            raise PermissionError("Seed 43 cannot depend on a later cached-head seed")
        return
    if seed != 44 or gate is None:
        raise PermissionError("Seed 44 requires the verified seed-43 formal gate")
    ...
    if (
        ...
        or run.get("stage") != formal_stage(43)
        or run.get("seed") != 43
        or run.get("test_split_accessed") is not False
    ):
        raise RuntimeError("Seed-43 formal prerequisite drift")
    if (
        verification.get("status") != "Passed"
        or verification.get("check_count") != 99
        or verification.get("failed_checks")
        or verification.get("test_split_accessed") is not False
    ):
        raise RuntimeError("Seed-43 formal prerequisite is not 99/99 Passed")
...
def verify_consumer_preflight(
    config: dict[str, Any], seed: int
) -> dict[str, Any] | None:
    gate = config.get("consumer_preflight")
    if config["stage"] == preflight_stage(seed):
        if gate is not None:
    ...
    if (
        run.get("status") != "Completed"
        or run.get("stage") != preflight_stage(seed)
        or run.get("seed") != seed
        or run.get("training_performed") is not False
        or run.get("performance_metrics_computed") is not False
        or run.get("test_split_accessed") is not False
    ):
        raise RuntimeError(f"Seed-{seed} consumer preflight run drift")
    if (
        verification.get("status") != "Passed"
        or verification.get("failed_checks")
        or verification.get("test_split_accessed") is not False
    ):
        raise RuntimeError(f"Seed-{seed} consumer preflight did not pass")
...
def base_validation(
    ...
    if initial_digest != config["execution"]["expected_head_initial_sha256"]:
        raise ValueError(f"Seed-{seed} head initialization hash drift")
...
def run_formal(
    *, config: dict[str, Any], shared: dict[str, Any], seed: int,
    ...
    preflight = verify_consumer_preflight(config, seed)
    validated = base_validation(config, shared, seed)
    ...
    if final_digest == validated["head_initial_sha256"]:
        raise ValueError(f"Seed-{seed} head did not update")
    private_manifest_path = private_dir / "private-manifest.json"
    private_manifest = {
        "experiment_id": "EXP-052",
        "stage": formal_stage(seed),
        "seed": seed,
    ...
    result.update(
        {
            ...
            "warnings": [
                f"Seed {seed} alone is not a frozen three-seed M2 result.",
...
def render_report(run: dict[str, Any]) -> str:
    seed = int(run["seed"])
    if run["stage"] == preflight_stage(seed):
        lines = [
            f"# EXP-052 Seed {seed} Cache-reuse Preflight", "",
        ...
        lines = [
            f"# EXP-052 M2 Cached Head - Seed {seed}", "",
            f"- Status: `{run['status']}`; independent verification pending",
...
def run() -> dict[str, Any]:
    ...
    private_dir = None
    if config["stage"] == formal_stage(args.seed):
        private_dir = (
    ...
    try:
        ...
        if config["stage"] == preflight_stage(args.seed):
