...
def load_config(path: Path, seed: int) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if authorization["seeds"] != [seed]:
        raise PermissionError("A formal run contract must authorize exactly one requested seed")
    ...
    amendment = run_config.get("authorization_amendment")
    if amendment is not None:
        require_hash(resolve_project(amendment["path"]), amendment["sha256"])
...
def verify_prior_seed_gate(run_config: dict[str, Any], seed: int) -> dict[str, Any] | None:
    gate = run_config.get("prior_seed_gate")
    if gate is None:
        return None
    if gate["seed"] >= seed:
        raise ValueError("Prior-seed gate must precede the authorized seed")
    ...
    verification = json.loads(verification_path.read_text(encoding="utf-8"))
    if (
        prior_run.get("experiment_id") != "EXP-051"
        or prior_run.get("seed") != gate["seed"]
        or prior_run.get("status") != "Completed"
    ):
        raise ValueError("Prior EXP-051 seed run is not complete")
    if (
        verification.get("experiment_id") != "EXP-051"
        or verification.get("seed") != gate["seed"]
        or verification.get("status") != gate["status"]
        or verification.get("failed_checks") != []
        or verification.get("check_count") != gate["check_count"]
    ):
        raise ValueError("Prior EXP-051 seed verification drift")
    if prior_run.get("test_split_accessed") is not False or verification.get("test_split_accessed") is not False:
        raise PermissionError("Prior seed gate does not prove test remained sealed")
    return {
        "seed": gate["seed"],
        "status": gate["status"],
        "verification_check_count": gate["check_count"],
        "run": artifact(run_path),
        "verification": artifact(verification_path),
...
def freeze_sources(
    run_dir: Path, config_path: Path, run_config: dict[str, Any]
) -> dict[str, Any]:
    sources = dict(FROZEN_FILES)
    sources["config"] = config_path.resolve()
    amendment = run_config.get("authorization_amendment")
    if amendment is not None:
        sources["authorization-amendment"] = resolve_project(amendment["path"])
    records = {}
...
def render_report(run_record: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# EXP-051 Seed {run_record['seed']} Validation",
            "",
            ...
            "",
            "This single-seed result cannot form the frozen three-seed M1 result.",
            "Only this seed was authorized by its run contract; test remained sealed.",
            "",
        ]
    )
...
def run() -> None:
    ...
    try:
        ...
        frozen_sources = freeze_sources(run_dir, args.config.resolve(), run_config)
        ...
        prior_seed_gate = verify_prior_seed_gate(run_config, args.seed)
        ...
        logger.info("Authorization, prerequisite gates, environment, and model hashes verified")
        ...
        run_record.update(
            {
                ...
                "prior_seed_gate": prior_seed_gate,
                ...
                "warnings": [
                    "One seed alone cannot form the frozen three-seed M1 conclusion.",
