...
def environment_gate(run_dir: Path, execution: dict[str, Any]) -> dict[str, Any]:
    ...
    if execution["required_device"] != "mps":
        raise ValueError("EXP-051 seed-42 contract requires MPS")
        ...
        run_record.update(
            {
                ...
                "warnings": [
                    ...
                    "MPS deterministic algorithms are requested with warn_only; bitwise repeatability is not claimed.",
                    "The shared threshold is selected on validation and cannot be tuned on test.",
                ],
            }
        )
        if run_record["resource_usage"]["peak_mps_driver_gb"] > run_config["execution"]["maximum_peak_memory_gb"]:
