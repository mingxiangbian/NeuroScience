...
def source_access_audit(path: Path) -> dict[str, bool]:
    ...
    split_arguments: list[str | None] = []
    test_path_literals = 0
    for node in ast.walk(tree):
        if isinstance(node, ast.Constant) and node.value == "test_path":
            test_path_literals += 1
        if not isinstance(node, ast.Call) or not isinstance(node.func, ast.Name):
            continue
        if node.func.id != "load_split" or len(node.args) < 2:
            continue
    ...
    return {
        ...
        "all_split_calls_static_and_allowlisted": bool(split_arguments)
        and all(split in ("train", "validation") for split in split_arguments),
        "no_test_path_lookup": '[f"{split}_path"]' in function_source and test_path_literals == 0,
        "no_recursive_data_access": all(token not in source for token in (".glob(", ".rglob(", "os.walk(")),
...
def verify() -> dict[str, Any]:
    ...
    if run.get("status") != "Completed":
        check(
            "test sealed",
            run.get("test_split_accessed") is False
            and config["authorization"]["test_access"] is False,
        )
        failed = [item["name"] for item in checks if not item["passed"]]
        verification = {
            "schema_version": "exp-052-m2-verification-v1",
            "experiment_id": "EXP-052",
            "stage": run.get("stage", config["stage"]),
            "seed": args.seed,
        ...
        summary_path.write_text(
            "\n".join(
                [
                    f"# EXP-052 Seed {args.seed} Verification",
                    "",
                    f"- Stage: `{verification['stage']}`",
                    "- Status: `Failed`",
                    f"- Run status: `{verification['run_status']}`",
                    f"- Failed checks: `{', '.join(failed)}`",
                    f"- Test accessed: `{'unknown' if run.get('test_split_accessed') is None else str(run.get('test_split_accessed')).lower()}`",
                    "",
                    "The run did not complete, so completed-run artifacts were not evaluated.",
                    "",
                ]
            ),
            encoding="utf-8",
