...
def run(config_path: Path, run_dir: Path) -> dict[str, Any]:
    ...
    reference_numpy = np.array(reference_logits.astype(mx.float32), copy=True)
    training_phase_peak_memory_gb = float(mx.get_peak_memory() / 1e9)
    ...
    replay_phase_peak_memory_gb = float(mx.get_peak_memory() / 1e9)
    ...
    peak_memory_gb = max(training_phase_peak_memory_gb, replay_phase_peak_memory_gb)
    resource_gate = {
    ...
    run_record = {
        ...
        "resources": {
            ...
            "training_phase_peak_mlx_memory_gb": training_phase_peak_memory_gb,
            "checkpoint_replay_phase_peak_mlx_memory_gb": replay_phase_peak_memory_gb,
            "phase_peak_rule": "max_of_sequential_training_and_checkpoint_replay_process_phases",
...
- Training / checkpoint-replay phase peak MLX memory: `{training_phase_peak_memory_gb:.3f}` / `{replay_phase_peak_memory_gb:.3f} GB`
- Resource-gate peak (maximum sequential phase peak): `{peak_memory_gb:.3f} GB`
