#!/usr/bin/env python3
"""Run the authorized EXP-053 seed-44 Classification LoRA train+validation gate."""
...
def load_config(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    expected = {
        ...
        "formal_training": True, "seed_44_authorized": True,
    ...
    if config.get("formal_execution_authorized") is not True:
        raise PermissionError("Formal EXP-053 seed 44 is not authorized")
...
def validate_prerequisites(config: dict[str, Any], shared: dict[str, Any]) -> dict[str, Any]:
    ...
    if prior_m3_seed43_run.get("status") != "Completed" or prior_m3_seed43_run.get("test_split_accessed"):
        raise RuntimeError("Prior EXP-053 seed 43 completion gate failed")
    if (
        prior_m3_seed43_verify.get("status") != "Passed"
        or prior_m3_seed43_verify.get("checks_total") != 143
        or len(prior_m3_seed43_verify.get("checks", [])) != 143
        or prior_m3_seed43_verify.get("failed_checks") != []
        or prior_m3_seed43_verify.get("test_split_accessed")
    ):
        raise RuntimeError("Prior EXP-053 seed 43 verification gate failed")
    ...
    if m2_run.get("status") != "Completed":
        raise RuntimeError("Matched EXP-052 seed 44 did not complete")
    require_passed_verification(m2_verify, 104, "Matched EXP-052 seed 44")
...
def initialize_model(shared: dict[str, Any], config: dict[str, Any], train_rows: Sequence[dict[str, Any]], validation_rows: Sequence[dict[str, Any]], logger: logging.Logger) -> dict[str, Any]:
    ...
    if head_initial != config["execution"]["expected_head_initial_sha256"]:
        raise ValueError("Seed-44 head initialization does not match M2")
...
def run(config_path: Path, run_dir: Path) -> dict[str, Any]:
    ...
    if time.perf_counter() - started > float(config["execution"]["maximum_total_wall_hours"]) * 3600:
        raise TimeoutError("Formal M3 exceeded frozen wall-time budget")
    logger.info("Formal seed 44 completed; independent verification is required")
    ...
    report = "\n".join([
        "# EXP-053 M3 Classification LoRA Seed 44", "",
