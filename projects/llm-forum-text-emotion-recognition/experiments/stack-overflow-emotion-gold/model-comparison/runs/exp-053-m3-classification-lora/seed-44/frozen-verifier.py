#!/usr/bin/env python3
"""Independently verify EXP-053 seed-44 Classification LoRA train+validation."""
...
DEFAULT_CONFIG = SCRIPT_DIR / "configs" / "exp-053-m3-seed-44.json"
DEFAULT_RUN_DIR = SCRIPT_DIR / "runs" / "exp-053-m3-classification-lora" / "seed-44"
...
def verify(config_path: Path, run_dir: Path) -> dict[str, Any]:
    ...
    check("authorized_seed", config["authorization"].get("seeds") == [SEED])
    ...
    check("seed44_authorized", config["authorization"].get("seed_44_authorized") is True)
    ...
    prior_m3_seed43_run = load_record(config["prerequisites"]["prior_m3_seed_43"]["run"])
    prior_m3_seed43_verify = load_record(config["prerequisites"]["prior_m3_seed_43"]["verification"])
    m2_run = load_record(config["prerequisites"]["m2_seed_44"]["run"])
    m2_verify = load_record(config["prerequisites"]["m2_seed_44"]["verification"])
    ...
    check("prior_m3_seed42_completed", prior_m3_run.get("status") == "Completed")
    check(
        "prior_m3_seed42_verified_148",
        prior_m3_verify.get("status") == "Passed"
        and prior_m3_verify.get("checks_total") == 148
        and len(prior_m3_verify.get("checks", [])) == 148
        and prior_m3_verify.get("failed_checks") == [],
    )
    check(
        "prior_m3_seed42_test_sealed",
        prior_m3_run.get("test_split_accessed") is False
        and prior_m3_verify.get("test_split_accessed") is False,
    )
    check("prior_m3_seed43_completed", prior_m3_seed43_run.get("status") == "Completed")
    check(
        "prior_m3_seed43_verified_143",
        prior_m3_seed43_verify.get("status") == "Passed"
        and prior_m3_seed43_verify.get("checks_total") == 143
        and len(prior_m3_seed43_verify.get("checks", [])) == 143
        and prior_m3_seed43_verify.get("failed_checks") == [],
    )
    check(
        "prior_m3_seed43_test_sealed",
        prior_m3_seed43_run.get("test_split_accessed") is False
        and prior_m3_seed43_verify.get("test_split_accessed") is False,
    )
    check("m2_seed44_completed", m2_run.get("status") == "Completed")
    check("m2_seed44_verified_104", passed_verification(m2_verify, 104))
    ...
    recomputed_bootstrap = {
        "fixed_0.5": bootstrap(gold, expected_fixed, component_ids, SEED, replicates, namespace, "fixed-0.5"),
        "shared_threshold": bootstrap(gold, expected_shared, component_ids, SEED, replicates, namespace, f"shared-{selected_threshold:.2f}"),
    }
    ...
    paired = {
        "comparison": "EXP-053_M3_seed44_minus_EXP-052_M2_seed44",
        "fixed_0.5": bootstrap(gold, expected_fixed, component_ids, SEED, replicates, namespace, "fixed-0.5", m2_fixed),
        "shared_threshold": bootstrap(gold, expected_shared, component_ids, SEED, replicates, namespace, f"M3-{selected_threshold:.2f}_M2-{m2_run['threshold_selection']['selected_threshold']:.2f}", m2_shared),
        "metric_deltas": {
    ...
    check("training_seed", run["training"]["seed"] == SEED)
    ...
    result = {
        ...
        "stage": STAGE, "verified_at_utc": utc_now(), "status": "Passed" if not failed else "Failed",
    ...
    summary = "\n".join([
        ...
        "- Test accessed: no", "",
        "This is the second of three seed verifications. Seed 44, EXP-054 and test remain sealed.", "",
    ])
