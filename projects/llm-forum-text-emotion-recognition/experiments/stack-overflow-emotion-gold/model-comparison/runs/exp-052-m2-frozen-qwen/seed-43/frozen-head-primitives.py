...
def verify_dry_run_gate(config: dict[str, Any]) -> dict[str, Any] | None:
    ...
    formal = config["stage"] == "seed-42-train-validation-integrity-gate"
    if gate is None:
        if formal:
            raise PermissionError("Formal EXP-052 execution requires a verified dry-run gate")
        return None
    if not formal:
        raise PermissionError("A dry-run contract must not depend on another dry-run gate")
...
def run_dry(
    *,
    ...
    projected_base_seconds = model_load_seconds + (
        extraction_seconds / len(selected)
