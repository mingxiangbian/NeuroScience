...
class CachedHeadContractTests(unittest.TestCase):
    ...
    def test_batch_orders_are_seeded_deterministic_and_seed_specific(self):
        ...
        seed_44 = runner.make_batch_orders(44, 20, 2)
        self.assertTrue(np.array_equal(first, second))
        self.assertFalse(np.array_equal(first, seed_44))
        for order in first:
            self.assertEqual(sorted(order.tolist()), list(range(20)))

    def test_stage_names_are_seed_specific_and_reject_unregistered_seeds(self):
        self.assertEqual(
            runner.preflight_stage(43), "seed-43-cache-reuse-preflight"
        )
        self.assertEqual(
            runner.formal_stage(44), "seed-44-cache-reuse-train-validation"
        )
        with self.assertRaises(PermissionError):
            runner.preflight_stage(42)
        with self.assertRaises(PermissionError):
            runner.formal_stage(45)
    ...
    def test_protocol_authorizes_only_seed_44_cached_head(self):
        protocol = (
            MODULE_DIR.parent
            / "protocols"
            / "exp-052-seed-44-cache-reuse-authorization.md"
        ).read_text(encoding="utf-8")
        self.assertIn("exactly", protocol)
        self.assertIn("three-seed aggregate", protocol)
        self.assertIn("does not authorize Stack Overflow test access", protocol)
        self.assertIn("Qwen model is not loaded", protocol)

    def test_config_authorization_when_frozen(self):
        paths = sorted((MODULE_DIR / "configs").glob("exp-052-m2-seed-*-cache-*.json"))
        if not paths:
            self.skipTest("Cached-head configs are frozen after implementation hashes exist")
        for path in paths:
            config = json.loads(path.read_text(encoding="utf-8"))
            seed = int(config["authorization"]["seeds"][0])
            self.assertEqual(config["experiment_id"], "EXP-052")
            self.assertIn(seed, runner.AUTHORIZED_CACHED_HEAD_SEEDS)
            self.assertEqual(config["authorization"]["splits"], ["train", "validation"])
            self.assertFalse(config["authorization"]["test_access"])
            self.assertEqual(config["authorization"]["seed_44_authorized"], seed == 44)
            self.assertFalse(config["authorization"]["exp_053_054_authorized"])
            self.assertFalse(
                config["authorization"].get("three_seed_aggregate_authorized", False)
            )
