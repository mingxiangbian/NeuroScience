...
def verify() -> dict[str, Any]:
    ...
    seed = args.seed
    expected_preflight = runner.preflight_stage(seed)
    expected_formal = runner.formal_stage(seed)
    ...
    check("seed identity", run["seed"] == seed and config["authorization"]["seeds"] == [seed])
    ...
    check("seed 44 authorization matches", config["authorization"]["seed_44_authorized"] is (seed == 44))
    check("M3 and M4 sealed", config["authorization"]["exp_053_054_authorized"] is False)
    check("three-seed aggregate sealed", config["authorization"].get("three_seed_aggregate_authorized", False) is False)
    ...
    if seed == 44:
        prior_run_path = verify_artifact(config["prior_seed_gate"]["run"])
        prior_verification_path = verify_artifact(config["prior_seed_gate"]["verification"])
        prior_run = json.loads(prior_run_path.read_text(encoding="utf-8"))
        prior_verification = json.loads(prior_verification_path.read_text(encoding="utf-8"))
        check(
            "seed-43 prerequisite completed",
            prior_run["status"] == "Completed"
            and prior_run["stage"] == runner.formal_stage(43)
            and prior_run["seed"] == 43,
        )
        check(
            "seed-43 prerequisite verified",
            prior_verification["status"] == "Passed"
            and prior_verification["check_count"] == 99
            and not prior_verification["failed_checks"],
        )
        check(
            "seed-43 prerequisite test sealed",
            prior_run["test_split_accessed"] is False
            and prior_verification["test_split_accessed"] is False,
        )
    else:
        check("seed-43 has no prior cached-head dependency", config.get("prior_seed_gate") is None)
    ...
    expected_head = prior_verifier.head_initial_digest(seed, 2560)
    ...
    expected_orders = runner.make_batch_orders(seed, len(train_rows), 2)
    ...
    formal = run["stage"] == expected_formal
    check("registered stage kind", run["stage"] in (expected_preflight, expected_formal))
    ...
    if not formal:
        ...
        for name, threshold, condition in (
            ...
            observed_bootstrap = prior_verifier.bootstrap_summary(
                gold, predicted, component_ids, seed,
        ...
        private_manifest = verify_artifact(run["artifacts"]["private_manifest"])
        check("private manifest Git ignored", gitignored(private_manifest))
        check("private predictions Git ignored", gitignored(predictions_path))
        check(
            "head-training budget",
            run["resource_usage"]["head_training_seconds"]
            <= float(config["execution"]["maximum_head_training_minutes"]) * 60,
        )
        check(
            "total wall-time budget",
            run["resource_usage"]["wall_seconds"]
            <= float(config["execution"]["maximum_total_wall_minutes"]) * 60,
        )
        check(
            "peak MLX memory budget",
            run["resource_usage"]["peak_mlx_memory_gb"]
            <= float(config["execution"]["maximum_peak_memory_gb"]),
        )
